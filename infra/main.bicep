...
resource raAppConfigContributor 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid(subscription().id, appConfiguration.id, functionApp.identity.principalId, 'Contributor')
...
resource raBlob 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid(subscription().id, storage.id, functionApp.identity.principalId, 'Storage Blob Data Contributor')
  scope: storage
  properties: {
...
resource raQueue 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid(subscription().id, storage.id, functionApp.identity.principalId, 'Storage Queue Data Contributor')
  scope: storage
  properties: {
